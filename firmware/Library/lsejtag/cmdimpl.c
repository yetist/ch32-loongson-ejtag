...
static uint32_t return_data_len_required(lsejtag_ctx *ctx) {
    ...
    case op_fast_target_mem_read: {
        ...
        uint32_t bytes_to_send = bytes_per_machine_word * read_cmd->data_len_xfer_count;
...
static uint32_t fast_mem_rw_get_bits_per_xfer(lsejtag_ctx *ctx) {
    const uint32_t core_count = ctx->core_count ? ctx->core_count : 1;
    ...
    if (ctx->use_fastdata) {
        ret = (ctx->is_64bit_cpu ? 71 : 39) + skipbits;
    } else {
        if (ctx->is_64bit_cpu) {
            ret = 118 + 2 * (core_count * ctx->param_ir_len_bits - 1 + skipbits);
        } else {
            ret = 86 + 2 * (core_count * ctx->param_ir_len_bits - 1 + skipbits);
        }
    }
...
lsejtag_cmd_exec_result cmdimpl_fast_mem_write(lsejtag_ctx *ctx, bool use_fastdata) {
    ...
    bits_per_xfer = fast_mem_rw_get_bits_per_xfer(ctx);
...
lsejtag_cmd_exec_result cmdimpl_continue_fast_mem_write(lsejtag_ctx *ctx) {
    struct lsejtag_jtagbuf_blk *bufblk;
    uint32_t *tdibuf, *databuf = (uint32_t *)ctx->buffered_cmd;
...
lsejtag_cmd_exec_result cmdimpl_fast_mem_read(lsejtag_ctx *ctx, bool use_fastdata) {
    lsejtag_cmd *cmd = (lsejtag_cmd *)ctx->buffered_cmd;
    struct lsejtag_jtagbuf_blk *bufblk = &ctx->jtagbuf_a; // In fastread we use buffer A exclusively
    uint32_t *tms_data = bufblk->tms_data, *tdi_data = bufblk->tdi_data;
    const uint32_t core_count = ctx->core_count ? ctx->core_count : 1;
    uint32_t skipbits;
    uint32_t bitshift = 0;
    ...
    ctx->cont_op = cmd->common.header.op;
    // Fast memory read doesn't have a GS232 mode, but for safety we still assume 1 core when core
    // count is not specified
    ctx->core_count = cmd->fast_mem_read_at_core.header.chained_core_count;
    ctx->core_count = ctx->core_count ? ctx->core_count : 1;
    
    // Calculate how many SKIP register bits needs to be considered when calculating bit length
    skipbits = ctx->core_count ? (ctx->core_count - 1) : 0;

    // This DR sequence length calculation is pretty much the same as fast memory write, because
    // it's all about dealing with the same DR thing
    ctx->drseq_bitlen = skipbits + (ctx->is_64bit_cpu ? 64 : 32) + (use_fastdata ? 1 : 0);
    ctx->drseq_dwordlen = BIT2DWORD(ctx->drseq_bitlen);

    // Due to the design limitation, one Run JTAG request can only yield one DR field, we can't do
    // burst read like fast memory write does. So TDI bit-length equals to bits_per_xfer.
    ctx->bits_per_xfer = fast_mem_rw_get_bits_per_xfer(ctx);
    ctx->tdi_bitlen = ctx->bits_per_xfer;

    // TDO skip bits are another thing to take care of. TDO read length equals drseq_bitlen.
    // With FASTDATA: just skip 4 bits where we try to select DR.
    // Without FASTDATA: skip (5 + (IRSeqBitLen) + 4) bits where we select IR=DATA and then DR.
    if (use_fastdata) {
        ctx->tdo_skip_bits = 4;
    } else {
        ctx->tdo_skip_bits = 9 + core_count * ctx->param_ir_len_bits;
    }
    ...
    // Reconfigure JTAG clock division as specified
    lsejtag_impl_reconfigure(impl_recfg_clkfreq, cmd->fast_mem_read_at_core.jtag_clk_div);
    ...
    // Prepare the TMS and TDI sequence
    memset(tms_data, 0, BIT2DWORD(ctx->tdi_bitlen) * 4);
    memset(tdi_data, 0, BIT2DWORD(ctx->tdi_bitlen) * 4);
    if (use_fastdata) {
        // Normal mode with FASTDATA & GS232 Mode: Just write DR, no IR writes
        // 0100 (Shift-DR)
        bitshift += append_bits(tms_data, bitshift, 0x2, 4);
    ...
    assert(bitshift == ctx->bits_per_xfer);

    // Clear command buffer.
    ctx->buffered_length = 0;

    // Set total data amount
    ctx->xfers_left = cmd->fast_mem_read_at_core.data_len_xfer_count;
...
lsejtag_cmd_exec_result cmdimpl_continue_fast_mem_read(lsejtag_ctx *ctx) {
    struct lsejtag_jtagbuf_blk *bufblk;

    // See if TDO buffer is full. If full, force TDO flush.
    if ((sizeof(ctx->tdo_in_data) / 4) - ctx->tdo_in_dwords < ctx->drseq_dwordlen) {
        ctx->tdo_immediate_flush = true;
        return cer_continue;
    }
    
    // Use buffer A exclusively
    if (!ctx->jtagbuf_a.busy) {
        bufblk = &ctx->jtagbuf_a;
    } else {
        return cer_await_jtag_buf;
    ...
    // Fill in transfer metadata
    bufblk->tdi_bits = ctx->tdi_bitlen;
    bufblk->tdo_bits = BIT2DWORD(ctx->drseq_bitlen) * 32; // Align to DWORDs
    bufblk->tdo_skip_bits = ctx->tdo_skip_bits;
