...
lsejtag_cmd_exec_result lsejtag_execute(lsejtag_ctx *ctx) {
    ...
    switch (cmd->common.header.op) {
        ...
        case op_fast_target_mem_read_fastdata:
            return cmdimpl_fast_mem_read(ctx, true);
        case op_fast_target_mem_read:
            return cmdimpl_fast_mem_read(ctx, false);
        case op_get_firmware_date:
...
static inline lsejtag_cmd_exec_result lsejtag_execute_continuation(lsejtag_ctx *ctx) {
    ...
    case op_fast_target_mem_read_fastdata:
    case op_fast_target_mem_read:
        return cmdimpl_continue_fast_mem_read(ctx);
...
void lsejtag_run_jtag(lsejtag_ctx *ctx) {
    ...
    uint32_t tdo_buf_mark = ctx->tdo_in_dwords;
    ...
    if (tdo_buf_mark + ((bufblk->tdo_bits + 31) >> 5) != ctx->tdo_in_dwords) {
        printf("PANIC: asked for %d bits of TDO, Watermark change: %d -> %d unexpected\n",
               bufblk->tdo_bits, tdo_buf_mark, ctx->tdo_in_dwords);
        while (1);
    }
    void dump_binary_to_console(const uint8_t * const data, uint32_t length);
    if (bufblk->tdo_bits) {
        // puts(">> TDO buf:");
        // dump_binary_to_console((uint8_t*)ctx->tdo_in_data, ctx->tdo_in_dwords * 4);
    }
